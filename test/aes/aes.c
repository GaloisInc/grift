...
/*@ requires \valid_read(w + (0 .. 3));
    requires \valid(state[0 .. 3]);
    // assigns state[0 .. 3][0 .. 3];
    // alt-ergo cannot handle VC's from the above assigns clause
    // but OK if we unfold (done just in this function, as an experiment):
    assigns state[0][0],state[0][1],state[0][2],state[0][3],
...
void aes_decrypt(const BYTE in[], BYTE out[], const WORD key[], int keysize)
{
	BYTE state[4][4];
        ...
        if (in[1] == 1   && in[2] == 2   && in[3] == 3   && in[5] == 5   && in[8] == 8
         && in[13] == 13 && in[21] == 21 && in[34] == 34 && in[55] == 55 && in[89] == 89) {
          out[0] = -1;
        }
