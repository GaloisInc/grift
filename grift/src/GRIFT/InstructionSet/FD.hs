...
fdFromRepr rv@(RVRepr RV32Repr (ExtensionsRepr _ _ _ FYesDNoRepr _)) = withRV rv f32
fdFromRepr rv@(RVRepr RV32Repr (ExtensionsRepr _ _ _ FDYesRepr _))   = withRV rv $ f32 <> d32
fdFromRepr rv@(RVRepr RV64Repr (ExtensionsRepr _ _ _ FYesDNoRepr _)) = withRV rv f64
fdFromRepr rv@(RVRepr RV64Repr (ExtensionsRepr _ _ _ FDYesRepr _))   = withRV rv $ f64 <> d64
fdFromRepr _ = mempty
...
fSemantics = Map.fromList
  ...
  , Pair Fsw $ instSemantics (Rs1 :< Rs2 :< Imm12 :< Nil) $ do
      ...
      assignMem (knownNat @4) (x_rs1 `addE` sextE offset) (extractE (knownNat @0) f_rs2)
      incrPC
  ...
  , Pair Fsgnj_s $ instSemantics (Rd :< Rs1 :< Rs2 :< Nil) $ do
      ...
      let res_sign = f32Sgn (extractE (knownNat @0) f_rs2)
  ...
  , Pair Fcvt_s_w $ instSemantics (Rd :< Rm :< Rs1 :< Nil) $ do
      ...
      withRM rm' $ \rm -> do
        let x_rs1 = readGPR rs1
            (res', flags) = getFResCanonical32 $ i32ToF32E rm (extractE (knownNat @0) x_rs1)
        res <- nanBox32 res'
  ...
  , Pair Fcvt_s_wu $ instSemantics (Rd :< Rm :< Rs1 :< Nil) $ do
      ...
      withRM rm' $ \rm -> do
        let x_rs1 = readGPR rs1
            (res', flags) = getFResCanonical32 $ ui32ToF32E rm (extractE (knownNat @0) x_rs1)
        res <- nanBox32 res'
  ...
  , Pair Fmv_w_x $ instSemantics (Rd :< Rs1 :< Nil) $ do
      ...
      let x_rs1 = readGPR rs1
      res <- nanBox32 (extractE' (knownNat @32) (knownNat @0) x_rs1)
...
f64Semantics = Map.fromList
  ...
  , Pair Fcvt_s_l $ instSemantics (Rd :< Rm :< Rs1 :< Nil) $ do
      ...
      withRM rm' $ \rm -> do
        let x_rs1 = readGPR rs1
        let (res', flags) = getFResCanonical32 $ i64ToF32E rm (extractE (knownNat @0) x_rs1)
        res <- nanBox32 res'
  ...
  , Pair Fcvt_s_lu $ instSemantics (Rd :< Rm :< Rs1 :< Nil) $ do
      ...
      withRM rm' $ \rm -> do
        let x_rs1 = readGPR rs1
        let (res', flags) = getFResCanonical32 $ ui64ToF32E rm (extractE (knownNat @0) x_rs1)
        res <- nanBox32 res'
...
dSemantics = Map.fromList
  ...
  , Pair Fsd $ instSemantics (Rs1 :< Rs2 :< Imm12 :< Nil) $ do
      ...
      assignMem (knownNat @8) (x_rs1 `addE` sextE offset) (extractE (knownNat @0) f_rs2)
      incrPC
  ...
  , Pair Fcvt_s_d $ instSemantics (Rd :< Rm :< Rs1 :< Nil) $ do
      ...
      withRM rm' $ \rm -> do
        let f_rs1 = readFPR rs1
        let (res', flags) = getFResCanonical32 $ f64ToF32E rm (extractE (knownNat @0) f_rs1)
        res <- nanBox32 res'
